...
"""
PTK Kernel — Patched
- Removes import-time side effects.
- Adds an energy ledger and conservation checks per step.
- Provides simulate_emission() returning result + ledger for analysis.
"""
...
class PTKKernel:
    ...
    def _init_rng(self, cfg: EmissionConfig):
    ...
    def simulate_emission(self, positive: List[Sound], negative: List[Sound], cfg: Optional[EmissionConfig] = None):
        ...
        ledger = {"input_energy": 0.0, "steps": [], "final": {}}
        ledger["input_energy"] = sum(s.energy for s in positive+negative)

        # Packets: [edge_id, progress, energy, freq, phase, coh, pol]
        packets: List[List[float]] = []

        def seed(sounds: List[Sound]):
            for s in sounds:
                ...
                for e in outs:
                    packets.append(
                        [e["id"], 0.0, share, s.frequency, s.phase, s.coherence, s.polarity])
        seed(positive)
        seed(negative)
        ...
        def emit_particle(node_id: str, E: float, coh: float):
            p = Particle(id=str(uuid.uuid4()), locus=node_id, energy=E,
                         mass2=E*(0.8+0.4*coh), Q=self._charges_from_node(node_id))
            particles.append(p)
            dets.append(DetectionEvent(id=str(uuid.uuid4()), kind="particle", ref_id=p.id,
                                       confidence=min(1.0, 0.6+0.4*coh)))

        def emit_field(edge_ids: List[str], E: float, coh: float):
            f = Field(id=str(uuid.uuid4()), support_edges=edge_ids[:32], energy=E,
                      strength=E*(0.5+0.5*coh), mode=("vector" if coh > 0.7 else "scalar"))
            fields.append(f)
            dets.append(DetectionEvent(id=str(uuid.uuid4()), kind="field", ref_id=f.id,
        ...
        while step < self.cfg.max_steps and packets and (len(particles)+len(fields) < self.cfg.max_outputs):
            ...
            energy_before = sum(p[2] for p in packets)
            for pk in packets:
                e = self.edge_index[pk[0]]
                pk[1] += self.cfg.step_len
                pk[2] *= self._edge_gain(e)
                by_edge.setdefault(pk[0], []).append(pk)

            new_packets: List[List[float]] = []
            cancelled_energy, field_energy, localized_energy = 0.0, 0.0, 0.0
            ...
            for edge_id, plist in by_edge.items():
                ...
                if P and N:
                    ...
                    while i < len(P) and j < len(N):
                        p, n = P[i], N[j]
                        df = abs(p[3]-n[3]) / max(1e-6, (p[3]+n[3])/2)
                        dphi = abs(((p[4]-n[4]+math.pi) % (2*math.pi))-math.pi)
                        if df <= self.cfg.cancel_bandwidth and dphi >= (math.pi-self.cfg.cancel_phase_tol):
                            k = self.cfg.cancel_efficiency*min(p[5], n[5])
                            dE = k*min(p[2], n[2])
                            p[2] -= dE
                            n[2] -= dE
                            cancelled_energy += dE*2
                            field_energy += dE*0.5
                        if p[3] <= n[3]:
                            i += 1
                        else:
                    ...
                    if field_energy >= self.cfg.field_E_thresh:
                        coh = sum(p[5] for p in P+N)/len(P+N)
                        emit_field([edge_id], field_energy, coh)
                ...
                for pk in plist:
                    ...
                    if pk[1] < 1.0:
                        ...
                        outs = self.next_edges.get((e["target"], pk[6]), [])
                        if not outs:
                            localized_energy += pk[2]
                            if pk[2] >= self.cfg.particle_E_thresh and pk[5] >= self.cfg.coherence_thresh:
                                emit_particle(e["target"], pk[2], pk[5])
                        else:
                            share = (pk[2]*self.cfg.split_decay)/len(outs)
                            for oe in outs:
                                new_packets.append(
                                    [oe["id"], 0.0, share, pk[3], pk[4], pk[5], pk[6]])
            packets = new_packets
            energy_after = sum(p[2] for p in packets)
            ...
            step_rec = {"step": step, "drift_rel": drift /
                        max(1e-12, energy_before)}
            if step_rec["drift_rel"] > self.cfg.conservation_tol:
                step_rec["conservation_warning"] = True
            ledger["steps"].append(step_rec)

        ledger["final"] = {"particles_energy": sum(p.energy for p in particles),
                           "fields_energy": sum(f.energy for f in fields),
                           "remaining_packets_energy": sum(p[2] for p in packets)}
        return {"particles": [asdict(p) for p in particles],
                "fields": [asdict(f) for f in fields],
                "detections": [asdict(d) for d in dets],
                "steps": step, "ledger": ledger}
